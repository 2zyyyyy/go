...
- 运行go get package[@version](https://github.com/version)将会升级到指定的版本号version
...
4.在你的目录文件下会生成`go.mod`和`go.sum`文件` go.mod `里面包含了你的所有的包！

5.在文件里面引入包名的时候有的编辑器会报错但是是可以正常编译的

6.下载包使用`go get`

7.修改包的版本号直接去`go.mod`文件修改然后`go mod download`
